...
let
  audioMonitorScript = pkgs.writeShellScript "volume-listener" ''
    ...
    until ${pkgs.pulseaudio}/bin/pactl get-sink-info @DEFAULT_SINK@ >/dev/null 2>&1; do
      sleep 0.5
    done

    until ${pkgs.pulseaudio}/bin/pactl get-source-info @DEFAULT_SOURCE@ >/dev/null 2>&1; do
      sleep 0.5
    done
